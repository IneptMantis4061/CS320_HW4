...
function cumulativeSum(input) {
  ...
  for (i = 0; i < input.length; i++) { // loop through each element of the input array
...
function maxPrimeSum(input) {
  ...
  for (i = 0; i < primes.length; i++) { // loop through the array closing in from the top down
    sum = 0; // initialize the sum to zero with each pass through the loop
    for (j = i; j < primes.length - i; j++) { // loop through the array adding up all previous elements of the array
